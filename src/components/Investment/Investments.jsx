...
export const Investments = ({ onChangeSlide }) => {
	const slides = [
		...
		},
		{
			...
			second_text: '',
			list: ['ROI проекта 20% годовых', 'Первый взнос 10%', 'Беспроцентная рассрочка', 'Возможность оплаты в криптовалюте USDT'],
		...
		},
		{
			...
			list: ['Мы полностью сопровождаем сделку до и после покупки. Подробнее в разделе Investor Journey', 'Наша экспертиза на местном рынке - ваше преимущество'],
		...
		},
		{
			...
			list: ['ЖК сдается полностью готовым для жизни. Квартиры с эффектным дизайном “под ключ”.', 'В комплексе есть все удобства: Infinity-бассейн на крыше, Sky-бар, тренажерный зал, спа-зона, детская зона, магазины, ресторан Мишлен и многое другое.'],
		},
		{
		...
		<div className='investment' style={{ backgroundImage: `url(${slides[currentSlide].background})` }}>
			<div className='container'>
				<div className='investment_section'>
					{currentSlide === 2 && <img className='investment_section-img' src='/images/icon/visa.svg' alt='' />}
					<div className='investment_section-text'>
						<h2>
							<span> {firstWord} </span>
							{restOfText}
						</h2>
						{currentSlide === 1 && <div className='second_icon'><img src='/images/icon/second_slide.svg' alt='Check Icon' /></div>}
						<p>{slides[currentSlide].second_text}</p>
						{slides[currentSlide].list && <div className='list'>{slides[currentSlide].list.map((item, index) => <li key={index}>{item}</li>)}</div>}
					</div>
					{currentSlide === 5 && (
